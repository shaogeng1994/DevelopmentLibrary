...
class SideSlideLayout(context: Context, attributeSet: AttributeSet? = null, defStyleAttr: Int = 0):
    ...
    var frontLayout: FrameLayout? = null
    var backLayout: FrameLayout? = null
    ...
    private fun measureChildren() {
        ...
        backLayout = getChildAt(0) as FrameLayout?
        frontLayout = getChildAt(1) as FrameLayout?
...
//        LogUtils.i("side", "onTouchEvent")
        var x = event?.x

        if (event?.action == MotionEvent.ACTION_DOWN) {
//            LogUtils.i("side", "down")
            startX = event?.x
            lastX = startX
            isFirstMeasure = true
            return true
        } else if (event?.action == MotionEvent.ACTION_MOVE) {
//            LogUtils.i("side", "move")
