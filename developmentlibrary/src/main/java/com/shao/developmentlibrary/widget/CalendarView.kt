...
//    var currentDay: Calendar? = null
    ...
    val textSize = resources.getDimensionPixelSize(R.dimen.dl_text_size_big)
    val per: Float = (resources.displayMetrics.widthPixels.div(750f))
    val calWidth = 670 * per
    val dayWidth = calWidth / 7f
    val dayHeight = 64 * per
    val days = ArrayList<Map<String, Any>>()
    val tipMap = HashMap<Int, Int>()
    ...
    override fun draw(canvas: Canvas?) {
        super.draw(canvas)
//        LogUtils.i("${mCalendar.get(Calendar.YEAR)}年 ${mCalendar.get(Calendar.MONTH)}月 ${mCalendar.get(Calendar.DAY_OF_MONTH)}")
...
//            canvas?.drawRect(p, mPaint)
    ...
    fun drawDayText(canvas: Canvas?, calendar: Calendar, rectF: RectF) {
    ...
    fun drawOtherDayText(canvas: Canvas?, calendar: Calendar, rectF: RectF) {
        val center = getRectCenter(rectF)
        val text = calendar.get(Calendar.DAY_OF_MONTH).toString()
        mPaint.textAlign = Paint.Align.CENTER
    ...
    fun getRectCenter(rectF: RectF): Array<Float> {
